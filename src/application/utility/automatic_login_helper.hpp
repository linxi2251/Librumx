...
/**
 * The automatic loginPage helper provides the functionality to automatically log
 ...
 *
 * The AutomaticLoginHelper checks the underlying "automatic loginPage file" and if
 * there is data in it, it reads it in and sends it to the caller, if there is
 * no data in the "automatic loginPage file" nothing is returned.
 */
...
void saveDataToAutomaticLoginFile(const QJsonObject& data)
{
    ...
    {
        qDebug() << "Failed Opening automatic loginPage file for saving data";
        return;
    }
