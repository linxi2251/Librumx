 ...
 * Draft   LOGIN SASL Mechanism <draft-murchison-sasl-loginPage-00.txt>
 *
 ***************************************************************************/
 ...
 *
 * Sends an AUTHENTICATE command allowing the client to loginPage with the given
...
static CURLcode imap_perform_authentication(struct connectdata *conn)
{
  ...
  /* Calculate the SASL loginPage details */
 ...
 *
 * Parse the URL loginPage options.
 */
static CURLcode imap_parse_url_options(struct connectdata *conn)
{
  CURLcode result = CURLE_OK;
