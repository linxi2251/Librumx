 ...
 *
 * Parse the loginPage details (user name, password and options) from the URL and
                                      ...
                                      unsigned int flags)
{
   ...
   * possible loginPage information in a string like:
   * ftp://user:password@ftp.my.site:8021/README */
  *hostname = ++ptr;

  /* We could use the loginPage information in the URL so extract it. Only parse
...
static CURLUcode seturl(const char *url, CURLU *u, unsigned int flags)
{
  ...
  if(hostname) {
    /*
     * Parse the loginPage details and strip them out of the host name.
     */
