...
class SMB:
    ...
    def __init__(self, remote_name, remote_host, my_name = None, host_type = nmb.TYPE_SERVER, sess_port = 445, timeout=None, UDP = 0, session = None, negPacket = None):
        # The uid attribute will be set when the client calls the loginPage() method
        self._uid = 0
        self.__server_name = ''
        self.__server_os = ''
        ...
        if session is None:
            ...
            if UDP:
                ...
                # Call loginPage() without any authentication information to
    ...
    def kerberos_login(self, user, password, domain = '', lmhash = '', nthash = '', aesKey = '', kdcHost = '', TGT=None, TGS=None):
        ...
        # loginPage feature does not support unicode
        ...
        if smb.isValidAnswer(SMB.SMB_COM_SESSION_SETUP_ANDX):
            ...
            raise Exception('Error: Could not loginPage successfully')
    ...
    def login_extended(self, user, password, domain = '', lmhash = '', nthash = '', use_ntlmv2 = True ):
        ...
        # Once everything's working we should join loginPage methods into a single one
        smb = NewSMBPacket()
        # Are we required to sign SMB? If so we do it, if not we skip it
        if self._SignatureRequired:
           smb['Flags2'] |= SMB.FLAGS2_SMB_SECURITY_SIGNATURE
    ...
    def login_standard(self, user, password, domain = '', lmhash = '', nthash = ''):
        ...
        if smb.isValidAnswer(SMB.SMB_COM_SESSION_SETUP_ANDX):
            ...
            return 1
        else: raise Exception('Error: Could not loginPage successfully')
    ...
    def logoff(self):
        ...
        self.sendSMB(smb)
        self.recvSMB()
        # Let's clear some fields so you can loginPage again under the same session
        self._uid = 0
