...
bool ValidFirstLine(const std::vector<RowScratchRegisters> *rows, int row,
  ...
  if (!StrongModel(model)) {
    tprintf("ValidFirstLine() should only be called with strong data_models!\n");
  }
...
bool ValidBodyLine(const std::vector<RowScratchRegisters> *rows, int row,
                   const ParagraphModel *model) {
  if (!StrongModel(model)) {
    tprintf("ValidBodyLine() should only be called with strong data_models!\n");
  }
...
bool CrownCompatible(const std::vector<RowScratchRegisters> *rows, int a, int b,
  ...
  if (model != kCrownRight && model != kCrownLeft) {
    tprintf("CrownCompatible() should only be called with crown data_models!\n");
    return false;
  }
  auto &row_a = (*rows)[a];
  auto &row_b = (*rows)[b];
...
void ParagraphModelSmearer::CalculateOpenModels(int row_start, int row_end) {
  ...
  for (int row = (row_start > 0) ? row_start - 1 : row_start; row < row_end; row++) {
    if ((*rows_)[row].ri_->num_words == 0) {
      ...
      // Which data_models survive the transition from row to row + 1?
...
void ParagraphModelSmearer::Smear() {
  ...
  for (int i = row_start_; i < row_end_; i++) {
    ...
    // Step One:
    //   Figure out if there are "open" data_models which are left-alined or
    ...
    if (likely_start) {
      // Add Start Hypotheses for all Open data_models that fit.
    ...
    //   data_models in our theory and see if this row could be the start
    //   of any of our  data_models.
    ...
    //   to recalculate the Open data_models for the rest of rows[i + 1, row_end)
    if (row.GetLineType() != LT_UNKNOWN) {
...
//   Forget any flush-{left, right} data_models unless we see two or more
//   of them in sequence.
//
...
// data_models for  everything,  it may be  due to an  inset paragraph like a
...
// have wildly different paragraph data_models, so we don't key any data off
// of those lines.
//
...
//       clues.
//   (3) Form data_models for any sequence of start + continuation lines.
//   (4) Smear the paragraph data_models to cover surrounding text.
...
//   data_models - the list of paragraph data_models referenced by the PARA objects.
//            caller is responsible for deleting the data_models.
void DetectParagraphs(int debug_level, std::vector<RowInfo> *row_infos,
  ...
  //   and geometric clues to form matching data_models for.  Let's see if
  ...
  // Undo any flush data_models for which there's little evidence.
